...
public class Device {
    ...
    private static void checkVulkanVersion(int apiVersion) {
        if (VK_VERSION_MAJOR(apiVersion) > 1 || 
           (VK_VERSION_MAJOR(apiVersion) == 1 && VK_VERSION_MINOR(apiVersion) >= 1)) {
            return;
        }
        throw new RuntimeException("Vulkan 1.1 is required to run the mod. Detected Vulkan version: " + VK_VERSION_MAJOR(apiVersion) + "." + VK_VERSION_MINOR(apiVersion));
    }
